...
public extension UIScrollView {
    ...
    func scrollToRevealFirstResponder(_ adjustInsets: @escaping (UIView) -> UIEdgeInsets = alignToRow) -> Disposable {
        ...
        bag += keyboardSignal(priority: .contentOffset).debounce(0.1).filter(predicate: { _ in self.window != nil }).filter { _ in self.firstResponder != nil }.onValue { event -> Void in
        ...
        bag += NotificationCenter.default.signal(forName: UITextField.textDidBeginEditingNotification).debounce(0.1).filter(predicate: { _ in self.window != nil }).filter { _ in self.firstResponder != nil }.onValue { _ in
